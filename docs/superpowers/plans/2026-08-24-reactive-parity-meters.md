...
- Sign conventions: production negative, consumption positive; +Q inductive/lagging. The PF driver derives `Q = |P| · tan(acos(pf))`, negated when `leading`; `pf` is true cos φ, valid in `(0, 1]` (0 rejected — division-free but meaningless; 1 → Q = 0 allowed). `:reactive-pf-limit` remains the ratio k = |Q|/|P| — different meaning, documented.
